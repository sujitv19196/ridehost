...
def run_executable(executable_path, num_ittr):
    ...
    for i in range(num_ittr):
        ...
        output = None
        try: 
            result = subprocess.run([executable_path, str(nodeType), "172.22.150.238", str(startLat), str(startLng), str(destLat), str(destLng)], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, timeout=10)
